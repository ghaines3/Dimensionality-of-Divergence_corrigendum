...
estimate.ED.eig <- function (eigen_val) {
  ...
  eig.out<-vector()
  eig.out["n1"] = prod(norm_eigen_val^(-norm_eigen_val))  # (Cangelosi & Goriely 2007, Roy & Vetterli 2007, Gnedenko & Yelnik 2016)
  eig.out["n2"] = (eigen_sum^2)/sum(eigen_val^2) # (Fraedrich et al. 1995, Bretherton et al. 1999, Pirkl et al. 2012)
  eig.out["nInf"] = eigen_sum/max(eigen_val)   # (Kirkpatrick et al. 2009) 
  eig.out["nC"] = K - ((K^2)/(eigen_sum^2))*eigen_var  # (Cheverud 2001), (Wagner et al. 2008)
  eig.out
}
